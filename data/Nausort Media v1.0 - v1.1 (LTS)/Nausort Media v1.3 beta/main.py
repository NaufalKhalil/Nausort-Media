"""
Nausort Media v1.3 — Media Expansion Update

=> Updates v1.3 :
- Drag & Drop folder support (drop from Windows Explorer)
- Video file support (.mp4, .mov, .avi, .mkv, .wmv, .m4v)
- Sorting summary report when all media is processed
- Photo / video / total media statistics
- Source folder status detection after sorting session
...
def _install_missing():
    ...
    if getattr(sys, "frozen", False):
        return
    for pkg in REQUIRED:
        mod = {"pywebview": "webview", "Pillow": "PIL", "windnd": "windnd"}.get(pkg, pkg)
        try:
            __import__(mod)
        except ImportError:
            print(f"[INSTALL] {pkg} ...")
...
# ── Imports ───────────────────────────────────────────────
import base64, io, json, os, queue, shutil, subprocess, threading, time
...
def _is_photo(path):
    return _ext_of(path) in PHOTO_EXTS

def _is_video(path):
    return _ext_of(path) in VIDEO_EXTS
...
def _scan_folder(folder):
    """Scan a flat source folder for supported media and count unsupported files."""
    photos, videos, unsupported = [], [], 0
    try:
...
def _analyze_source_folder(folder):
    """Count remaining supported media and unsupported files in the source folder."""
    remaining = 0
    unsupported = 0
    if not folder or not os.path.isdir(folder):
        return {"remaining": 0, "unsupported": 0, "is_clean": True}
    try:
        for name in os.listdir(folder):
            path = os.path.join(folder, name)
            if not os.path.isfile(path):
                continue
            if _ext_of(path) in SUPPORTED_EXTS:
                remaining += 1
            else:
                unsupported += 1
    except Exception:
    ...
    return {
        "remaining": remaining,
        "unsupported": unsupported,
        "is_clean": remaining == 0 and unsupported == 0,
    }

def _resolve_drop_paths(paths):
    """Validate dropped paths; return (folder_path, error_key) or (path, None)."""
    if not paths:
        return None, "empty"
    ...
    if len(cleaned) == 1:
        ...
        if os.path.isdir(p):
            return p, None
        return None, "file"
    parent_dirs = {os.path.dirname(os.path.abspath(p)) for p in cleaned}
...
class AppState:
    def __init__(self):
        ...
        # v1.3 session statistics for sorting summary
        self.session_photos_sorted = 0
        self.session_videos_sorted = 0
        self.session_bytes          = 0
        self.session_folders        = set()
        self.initial_unsupported    = 0
    ...
    def record_sort(self, src, dst_dir):
        try:
            self.session_bytes += os.path.getsize(src)
        except Exception:
            pass
        if _is_photo(src):
            self.session_photos_sorted += 1
        elif _is_video(src):
            self.session_videos_sorted += 1
        self.session_folders.add(os.path.abspath(dst_dir))

    def record_undo_sort(self, rec):
        src = rec.get("src", "")
        try:
            self.session_bytes -= os.path.getsize(rec.get("dst", src))
        except Exception:
            pass
        if _is_photo(src):
            self.session_photos_sorted = max(0, self.session_photos_sorted - 1)
        elif _is_video(src):
            self.session_videos_sorted = max(0, self.session_videos_sorted - 1)

    def record_redo_sort(self, rec):
        src = rec.get("src", "")
        dst_dir = os.path.dirname(rec.get("dst", ""))
        try:
            self.session_bytes += os.path.getsize(src)
        except Exception:
            pass
...
class NausortAPI:
    ...
    def _build_session_summary(self):
        status = _analyze_source_folder(_state.source_folder)
        total_sorted = _state.session_photos_sorted + _state.session_videos_sorted
        summary = {
            "photos_sorted":      _state.session_photos_sorted,
            "videos_sorted":      _state.session_videos_sorted,
            "total_media":        total_sorted,
            "folders_created":    len(_state.session_folders),
            "unsupported_files":  _state.initial_unsupported + status["unsupported"],
            "source_remaining":   status["remaining"],
            "source_unsupported": status["unsupported"],
            "source_clean":       status["is_clean"],
            "total_data":         _format_bytes(max(0, _state.session_bytes)),
        }
        _log("=" * 31, "dim")
        _log("Proses Selesai", "info")
        _log("=" * 31, "dim")
        _log(f"Foto Disortir       : {summary['photos_sorted']}", "info")
        _log(f"Video Disortir      : {summary['videos_sorted']}", "info")
        _log(f"Total Media         : {summary['total_media']}", "info")
        _log(f"Folder Dibuat       : {summary['folders_created']}", "info")
        _log(f"File Tidak Didukung : {summary['unsupported_files']}", "info")
        if summary["source_clean"]:
            _log("Status              : Folder sumber sudah kosong", "info")
        else:
            _log("Status              : Folder sumber masih berisi file", "warn")
            _log(f"File Tersisa        : {summary['source_remaining']}", "warn")
            _log(f"File Tidak Didukung : {summary['source_unsupported']}", "warn")
        _log(f"Total Data          : {summary['total_data']}", "info")
        return summary

    def _apply_folder_load(self, folder, scan):
        _state.source_folder  = folder
        _state.photo_list     = scan["media"]
        _state.current_index  = 0 if scan["media"] else -1
        _state.history.clear()
        _state.redo_stack.clear()
        _state.reset_session_stats()
        _state.initial_unsupported = scan["unsupported"]
        ...
        _log(
            f"[INFO] {scan['photo_count']} photo(s), "
            f"{scan['video_count']} video(s) found",
            "info",
        )
        if scan["unsupported"]:
            _log(f"[INFO] {scan['unsupported']} unsupported file(s) in folder", "info")
        ...
        if scan["media"]:
            ...
            result.update(self._get_current_image_data())
        return result

    def _load_folder_from_path(self, folder):
        if not folder or not os.path.isdir(folder):
            return {"ok": False, "error": "Folder tidak dapat diakses."}
        if not _state._import_lock.acquire(blocking=False):
            return {"ok": False, "error": "Import sedang berjalan."}
        try:
            scan = _scan_folder(folder)
            if scan is None:
                return {"ok": False, "error": "Folder tidak dapat diakses."}
            if not scan["media"]:
                return {"ok": False, "error": "Tidak ditemukan media yang didukung."}
            return self._apply_folder_load(folder, scan)
        finally:
            _state._import_lock.release()

    def get_init_data(self):
        notice = _state.config_notice
        _state.config_notice = None  # only show once
        photo_count = sum(1 for p in _state.photo_list if _is_photo(p))
        video_count = sum(1 for p in _state.photo_list if _is_video(p))
        return {
            "categories":    _state.config.get("categories", []),
            "photo_count":   photo_count,
            "video_count":   video_count,
            "media_count":   len(_state.photo_list),
            "current_index": _state.current_index,
    ...
    def import_folder(self):
        ...
        return self._load_folder_from_path(folder)

    def import_folder_from_path(self, folder_path):
        return self._load_folder_from_path(folder_path)

    def handle_dropped_paths(self, paths):
        global _drop_debounce_until
        now = time.time()
        if now < _drop_debounce_until:
            return {"ok": False, "silent": True}
        _drop_debounce_until = now + 2.0

        folder, err = _resolve_drop_paths(paths)
        if err == "file":
            return {"ok": False, "error": "Hanya folder yang dapat digunakan."}
        if err == "multi":
            return {"ok": False, "error": "Hanya satu folder yang dapat digunakan."}
        if err == "shortcut":
            return {"ok": False, "error": "Hanya folder yang dapat digunakan."}
        if err == "empty" or not folder:
            return {"ok": False, "error": "Hanya folder yang dapat digunakan."}
        result = self._load_folder_from_path(folder)
        if result.get("ok"):
            result["toast"] = "Folder berhasil dimuat."
        return result

    def _media_counts(self):
        photos = sum(1 for p in _state.photo_list if _is_photo(p))
        videos = sum(1 for p in _state.photo_list if _is_video(p))
        return photos, videos, len(_state.photo_list)

    def _get_current_image_data(self):
        photos, videos, total = self._media_counts()
        base_counts = {"photo_count": photos, "video_count": videos, "total": total}
        idx = _state.current_index
        if not (0 <= idx < len(_state.photo_list)):
            return {"filename": "", "image_data": "", "index": -1, **base_counts}
            ...
            if _is_video(path):
                try:
                    if not os.path.isfile(path) or os.path.getsize(path) <= 0:
                        raise OSError("invalid video file")
                except Exception as e:
    ...
    def sort_to(self, category_index, conflict_action=None):
        ...
        with _state._busy_lock:
            ...
            _state.record_sort(src, dst_dir)
        ...
        result = {"ok": True, **self._get_current_image_data()}
        if not _state.photo_list:
            result["session_complete"] = True
            result["summary"] = self._build_session_summary()
        return result
...
# ── Drag & drop (pywebview DOM events) ────────────────────
def _bind_drag_drop(window, api):
    try:
        from webview.dom import DOMEventHandler
    except ImportError:
        return

    def _notify_drop_highlight(active):
        if not _window:
            return
        try:
            val = "true" if active else "false"
            _window.evaluate_js(f"window._setDropHighlight({val})")
        except Exception:
            pass

    def _notify_folder_imported(res_json):
        if not _window:
            return
        try:
            safe = res_json.replace("\\", "\\\\").replace("`", "\\`")
            _window.evaluate_js(f"window._onFolderDropped(`{safe}`)")
        except Exception:
            pass

    def on_drag(e):
        _notify_drop_highlight(True)

    def on_drag_leave(e):
        _notify_drop_highlight(False)
    ...
    def on_drop(e):
        ...
        if not paths:
            return
        result = api.handle_dropped_paths(paths)
        try:
            _notify_folder_imported(json.dumps(result, ensure_ascii=False))
        except Exception:
            pass

    window.dom.document.events.dragenter += DOMEventHandler(on_drag, True, True)
    window.dom.document.events.dragover  += DOMEventHandler(on_drag, True, True, debounce=100)
    window.dom.document.events.dragleave += DOMEventHandler(on_drag_leave, True, True)
    window.dom.document.events.drop      += DOMEventHandler(on_drop, True, True)

def _bind_windnd_drop(window, api):
    """Windows native drop fallback — pywebviewFullPath may not include folders."""
    if platform.system() != "Windows":
        return
    try:
        import windnd
    ...
    def _notify_folder_imported(result):
        if not _window:
            return
        try:
    ...
    def _on_drop_files(paths):
        ...
        for p in paths:
            ...
            decoded.append(p)
        result = api.handle_dropped_paths(decoded)
        _notify_folder_imported(result)
    ...
    def _hook():
        ...
        try:
            ...
            try:
                ...
                return
        try:
            windnd.hook_dropfiles(hwnd, func=_on_drop_files)
        except Exception:
            pass

    threading.Thread(target=_hook, daemon=True, name="windnd-hook").start()
...
def main():
    ...
    def bind_events(window):
        _bind_drag_drop(window, api)
        _bind_windnd_drop(window, api)
    ...
    webview.start(bind_events, _window, **start_kwargs)
