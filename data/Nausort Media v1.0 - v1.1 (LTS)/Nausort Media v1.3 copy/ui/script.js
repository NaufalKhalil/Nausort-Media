...
// ── Drag & Drop ──────────────────────────────────────────
// Real filesystem paths for dropped items are only ever available on the
// Python side (pywebview's window.dom drop bridge - see _setup_drag_drop
// in main.py). The listeners below only do the visual part: highlight the
// hovered drop target, show a contextual hint near the cursor, and stop
// the browser from navigating away on drop. window._dropZone records
// *which* target is currently hovered so the Python handler can ask the
// page (via evaluate_js) what to do with the path it received.
window._dropZone = null;

function _setDropZone(zone, label, x, y) {
  window._dropZone = zone;
  _showDropHint(label, x, y);
}
function _clearDropZone() {
  window._dropZone = null;
  _hideDropHint();
}
function _showDropHint(label, x, y) {
...
function initDragDrop() {
  ...
  viewer.addEventListener('dragenter', e => {
    e.preventDefault();
    viewer.classList.add('drag-over-import');
    _setDropZone({ type: 'import' }, 'Drop folder di sini untuk import', e.clientX, e.clientY);
  });
  viewer.addEventListener('dragover', e => {
    e.preventDefault();
    _showDropHint('Drop folder di sini untuk import', e.clientX, e.clientY);
  });
  ...
  viewer.addEventListener('drop', e => {
    ...
    viewer.classList.remove('drag-over-import');
    _clearDropZone();
    // The actual import happens on the Python side (window.dom drop
    // bridge); this just prevents the browser's default "open file" and
    // clears the visual feedback.
  });

  // -- Drop onto a category button: set its destination folder --
  catWrap.addEventListener('dragenter', e => {
    const btn = e.target.closest('.cat-btn');
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    // Clear any import-zone state the viewer may have set while the cursor
    // was passing over it on the way to a category button.
    viewer.classList.remove('drag-over-import');
    btn.classList.add('drag-over-cat');
    const index = Array.from(catWrap.children).indexOf(btn);
    _setDropZone({ type: 'category', index }, 'Drop folder di sini untuk mengatur destination', e.clientX, e.clientY);
  });
  catWrap.addEventListener('dragover', e => {
    const btn = e.target.closest('.cat-btn');
    if (!btn) return;
    e.preventDefault();
    e.stopPropagation();
    _showDropHint('Drop folder di sini untuk mengatur destination', e.clientX, e.clientY);
  });
  catWrap.addEventListener('dragleave', e => {
    const btn = e.target.closest('.cat-btn');
    if (btn && e.target === btn) {
      btn.classList.remove('drag-over-cat');
      _clearDropZone();
    }
  });
  catWrap.addEventListener('drop', e => {
    const btn = e.target.closest('.cat-btn');
...
window._handleImportResult = function(res) {
  if (!res || !res.ok) {
    if (res && res.error) showToast(res.error, false);
    return;
  }
  S.total        = res.total ?? S.total;
  S.currentIndex = res.current_index ?? S.currentIndex;
  updateStats();
  if (res.image_data) showImage(res); else showPlaceholder();
};

// Called from Python after a folder dropped onto a category button has
// been set as that category's destination.
window._handleCategoryFolderResult = function(res) {
  if (!res || !res.ok) {
    showToast((res && res.error) || 'Gagal mengatur folder tujuan', false);
    return;
  }
  S.categories = res.categories;
  renderCategories();
  showToast(res.toast || 'Destination diperbarui', true);
};
...
async function doTrash() {
  if (S.busy || S.total === 0) return;
  S.busy = true;
  setCatButtonsBusy(true);
  try {
    const res = await window.pywebview.api.delete_current();
    if (!res) { showToast('Gagal memindahkan ke Trash', false); return; }
    if (!res.ok) { showToast(res.error || 'Gagal memindahkan ke Trash', false); return; }
    S.total = res.total ?? S.total;
    if (res.image_data) showImage(res);
